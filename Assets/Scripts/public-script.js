...
const participantTypeSelector = document.getElementById(
...
function setupRegistrationFlow(event) {
  ...
  if (audience === "students_and_faculty") {
    participantTypeSelector.innerHTML = `
        ...
        `;
    document
      .getElementById("student-choice-btn")
      .addEventListener("click", () => {
        generateRegistrationForm(event, "student");
      });
    document
      .getElementById("faculty-choice-btn")
      .addEventListener("click", () => {
  ...
  if (customQuestions && customQuestions.length > 0) {
    ...
    customQuestions.forEach((q) => {
      ...
      if (q.type === "text") {
        ...
        for (let j = 1; j <= 10; j++)
          questionsHTML += `<option value="${j}">${j}</option>`;
...
function generateRegistrationForm(event, participantCategory) {
    ...
    finalHTML += `
            <div class="participant">
                ...
                <div class="fields text-center">
                    ...
                    <h5 class="mt-2"><strong>Event Fee: ₹${
                      event.eventFee
                    }</strong></h5>
                    <img src="${
                      event.qrCodeURL
                    }" alt="Payment QR Code" style="max-width: 250px; border-radius: 8px;" class="mb-3">
                    ${upiLinkHTML}
  ...
  const participantLabelText = event.paymentsEnabled
  ...
  regForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (eventData.paymentsEnabled) {
        ...
        const screenshotRef = storage.ref(
          `screenshots/${Date.now()}_${screenshotFile.name}`
        );
      ...
      const collectionName = `${eventData.eventName.replace(
        /\s+/g,
        ""
      )}${collectionSuffix}`;
      ...
      const successTitle = eventData.paymentsEnabled
