        ...
        const row = `<tr>
                ...
                <td>
                    <div class="action-buttons-container">
                        ...
                        <button class="btn btn-sm btn-info activate-btn" data-id="${eventId}" ${
          event.isActive ? "disabled" : ""
        }>Activate</button>
                        <button class="btn btn-sm btn-warning toggle-status-btn" data-id="${eventId}">${
...
function populateCustomQuestions(containerId, questions) {
  ...
  if (questions && questions.length > 0) {
    questions.forEach((q) => {
      ...
      container.querySelector('.border:last-child [data-type="type"]').value =
        q.type;
    });
  }
}
...
async function populateFormForEdit(eventId) {
  try {
    ...
    if (event.posterURL) {
      document.getElementById("current-poster-container").style.display =
        "block";
    ...
    populateCustomQuestions(
      "student-questions-container",
      event.studentCustomQuestions
    );
    populateCustomQuestions(
      "faculty-questions-container",
      event.facultyCustomQuestions
    );
    ...
    document.getElementById("confirmationEmailContent").value =
      event.confirmationEmailTemplate || "";
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const teamSettingsContainer = document.getElementById(
  ...
  const eventAudienceSelect = document.getElementById("eventAudience");
  const studentQuestionsSection = document.getElementById(
    "student-questions-section"
  );
  const facultyQuestionsSection = document.getElementById(
    "faculty-questions-section"
  );
  const addStudentQuestionBtn = document.getElementById(
    "add-student-question-btn"
  );
  const addFacultyQuestionBtn = document.getElementById(
    "add-faculty-question-btn"
  );
  ...
  if (eventAudienceSelect) {
    eventAudienceSelect.addEventListener("change", (e) => {
      ...
      studentQuestionsSection.style.display =
        audience === "students_only" || audience === "students_and_faculty"
          ? "block"
          : "none";
      facultyQuestionsSection.style.display =
        audience === "faculty_only" || audience === "students_and_faculty"
          ? "block"
          : "none";
    });
    eventAudienceSelect.dispatchEvent(new Event("change"));
  ...
  if (participationType) {
    participationType.addEventListener("change", (e) => {
      teamSettingsContainer.style.display =
  ...
  if (addStudentQuestionBtn) {
    addStudentQuestionBtn.addEventListener("click", () =>
      addQuestion("student-questions-container")
    );
  }
  if (addFacultyQuestionBtn) {
    addFacultyQuestionBtn.addEventListener("click", () =>
      addQuestion("faculty-questions-container")
    );
  }
  ...
  if (eventForm) {
    eventForm.addEventListener("submit", async (e) => {
      ...
      function getCustomQuestions(containerId) {
        return Array.from(document.querySelectorAll(`#${containerId} .border`))
          .map((q) => ({
      ...
      const eventData = {
        ...
        confirmationEmailTemplate: document.getElementById(
          "confirmationEmailContent"
        ).value,
        studentCustomQuestions: getCustomQuestions(
          "student-questions-container"
        ),
        facultyCustomQuestions: getCustomQuestions(
          "faculty-questions-container"
        ),
      ...
      try {
        if (eventPosterFile) {
          const posterRef = storage.ref(
            `event_posters/${Date.now()}_${eventPosterFile.name}`
          );
        ...
        if (eventData.paymentsEnabled && qrCodeFile) {
          const qrRef = storage.ref(
            `qr_codes/${Date.now()}_${qrCodeFile.name}`
          );
  ...
  if (pastEventsForm) {
    pastEventsForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const submitButton = pastEventsForm.querySelector(
        'button[type="submit"]'
      );
      submitButton.disabled = true;
      submitButton.textContent = "Saving...";
      for (let i = 1; i <= 3; i++) {
        ...
        const posterFile = document.getElementById(`pastEventPoster${i}`)
          .files[0];
        if (title && date && posterFile) {
          try {
            const storageRef = storage.ref(
              `past_event_posters/${Date.now()}_slot${i}_${posterFile.name}`
            );
            ...
            Swal.fire(
              "Error",
              `Error uploading Slot ${i}: ${err.message}`,
              "error"
            );
          }
        }
